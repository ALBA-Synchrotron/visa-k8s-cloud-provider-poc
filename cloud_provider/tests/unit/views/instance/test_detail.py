...
# forms test
class InstanceDetailViewTest(GenericApiViewTest):
    ...
    def test_specific_instance(self) -> None:
        ...
        response = self.client.get(f"{self.url}/{self.existing_object}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.get(f"{self.url}/{self.non_existent_id}")
