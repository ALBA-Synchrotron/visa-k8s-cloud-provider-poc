...
# forms test
class InstanceSecGroupRemViewTest(GenericApiViewTest):
    logger: logging.Logger = logging.getLogger(__name__)
    fixtures: list = ["image.json", "flavour.json", "security_group.json", "instance.json"]

    def setUp(self) -> None:
        super(InstanceSecGroupRemViewTest, self).setUp()
        ...
        self.url: str = "/v1.21.24/api/instances/{}/security_groups/remove"
    ...
    def test_remove_instance_security_groups(self) -> None:
        body: dict = {"name": "Test Name"}
        response = self.client.post(self.url.format(self.existing_object), data=json.dumps(body),
                                    content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        response = self.client.post(self.url.format(self.non_sec_group_object), data=json.dumps(body),
                                    content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        response = self.client.post(self.url.format(self.non_existent_id), data=json.dumps(body),
                                    content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        invalid_body: dict = {"name": "Test Name WASUP"}
        response = self.client.post(self.url.format(self.non_sec_group_object), data=json.dumps(invalid_body),
                                    content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
